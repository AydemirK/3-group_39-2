...
class AsyncDatabase:
    ...
    async def create_tables(self):
        async with aiosqlite.connect(self.db_path) as db:
            ...
            await db.execute(sql_queries.CREATE_DISLIKE_TABLE_QUERY)
            await db.execute(sql_queries.CREATE_REFERENCE_TABLE_QUERY)
            await db.execute(sql_queries.CREATE_SALE_QUERY)

            try:
                await db.execute(sql_queries.ALTER_TABLE_USER_QUERY)
                await db.execute(sql_queries.ALTER_TABLE_USER_BALANCE_QUERY)
